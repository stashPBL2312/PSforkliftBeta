// Safe per-row mapping: update archive_workshop_jobs using original job id and forklift_id
const fs = require('fs');
...
async function run(){
  ...
  for (const r of fkRows){
    ...
    mapOldToIdentity.set(oldId, {
      eq_no: normStr(r.eq_no),
      serial: normStr(r.serial_number || r.serial)
    });
  }

  console.log(`Reading workshop CSV: ${WORKSHOP_CSV}`);
  ...
  const jobs = wsRows.map(r => ({
    id: normIntOrNull(r.id),
    old_fk: normIntOrNull(r.forklift_id),
  ...
  let updated = 0, skipped = 0, unmapped = 0; const errors = [];
  ...
  try{
    ...
    for (const job of jobs){
      try{
        ...
        // Update only per job id (overwrite current wrong mapping)
        const res = await runAsync('UPDATE archive_workshop_jobs SET forklift_id=? WHERE id=?', [newFk, job.id]);
  ...
  console.log(`Safe mapping selesai. Rows updated: ${updated}, Unmapped jobs: ${unmapped}, Skipped: ${skipped}, Errors: ${errors.length}`);
